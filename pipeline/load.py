...
def load():
    try:
        ...
        bangunan = gpd.read_file(f'{DIR_TEMP_DATA}/public.Bangunan.geojson')
        pengawasan = pd.read_csv(f'{DIR_TEMP_DATA}/public.Pegawasan_dan_penindakan.geojson')
        
        
        # Load to public schema
        # Load category tables    
        bangunan.to_postgis('bangunan', 
                            con = dwh_engine, 
                            if_exists = 'append', 
                            index = False, 
                            schema = 'public')
